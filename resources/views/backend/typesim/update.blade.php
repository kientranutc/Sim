	...
	<div class="row">
		<div class="col-md-12">
			<div class="panel panel-default">
				<form class="no-margin" id="formValidate1" action="{{URL::route('type-sim.update',[$dataEdit->id])}}" method="post">
						 {{ csrf_field() }}
					<div class="panel-heading">Cập nhật loại sim</div>
					<div class="panel-body">
						<div class="row">
							<div class="col-md-6">
								<div class="form-group {{($errors->has('name'))?"has-error":""}} ">
									<label for="name" class="control-label">Name</label> <input
										type="text" placeholder="name" name="name" value="{{(old('name'))?old('name'):$dataEdit->name}}"
							...
							<div class="col-md-6">
								<div class="form-group  ">
									...
									@foreach($dataNet as $item)
									 <option value="{{$item->id}}" {{($dataEdit->net_id==$item->id)?'selected':''}} {{(old('net_id')==$item->id)?'selected':''}}>{{$item->name}}</option>
									@endforeach
									</select>
								</div>
							</div>
						...
						<div class="form-group {{($errors->has('image'))?"has-error":""}}">
									<label class="control-label">Image</label> <input
										type="text" placeholder="click để thêm ảnh" id="image-typesim" name="image" value="{{(old('image')?old('image'):$dataEdit->image)}}"
										class="form-control input-sm" data-required="true"
										>
										<p class="text-danger">{{$errors->first('image')}}</p>
								 <img  width="100px" height="100px" src="{{$dataEdit->image}}"  alt="{{$dataEdit->image}}">
								</div>
