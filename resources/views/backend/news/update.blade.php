	...
	<div class="row">
		<div class="col-md-12">
			<div class="panel panel-default">
				<form class="no-margin" id="formValidate1" action="{{URL::route('news.update',[$dataNews->id])}}" method="post">
						 {{ csrf_field() }}
					...
					<div class="panel-body">
						<div class="row">
							<div class="col-md-6">
								<div class="form-group {{($errors->has('title'))?"has-error":""}} ">
									<label for="name" class="control-label">Tiêu đề bản tin</label> <input
										type="text" placeholder="Tiêu đề bản tin" name="title" value="{{$dataNews->title}}"
							...
							<div class="col-md-6">
									...
									<label class="control-label">Ảnh đại diện bản tin</label> <input
										type="text" placeholder="click để thêm ảnh" id="image-typesim" name="image" value="{{$dataNews->image}}"
										class="form-control input-sm" data-required="true">
							...
							<div class="col-md-6">
								<div class="form-group {{($errors->has('net_id'))?"has-error":""}}">
									<label class="control-label">Tin theo nhà mạng</label>
										...
										<option value="{{$items->id}}" {{($dataNews->net_id == $items->id)?'selected':''}}>{{$items->name}}</option>
										@endforeach
										</select>
								...
								<div class="form-group {{($errors->has('status'))?"has-error":""}}">
									<label class="control-label">Trạng thái bản tin</label>
										<select class="form-control input-sm" name="status">
    										<option value="1" {{($dataNews->status==1)?'selected':''}}>Hiển thị</option>
    										<option value="0" {{($dataNews->status==0)?'selected':''}}>Ẩn</option>
										</select>
						...
						<div class="form-group {{($errors->has('description'))?"has-error":""}}">
									...
									<textarea  rows="" class="form-control input-sm" cols="" name="description">{{$dataNews->description}}</textarea>
						</div>
					</div>
